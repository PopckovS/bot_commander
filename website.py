...
from models.Telegram_Type import Telegram_Type
...
def page_index():
    ...
    user = db.session.query(Telegram_User).\
...
def website_server_start():
    """Метод запускает работу сервера и сайт на нем."""
    # if __name__ == "__main__":
    # app.run(debug=True, host=config.WEBSITE_HOST, port=config.WEBSITE_PORT)
